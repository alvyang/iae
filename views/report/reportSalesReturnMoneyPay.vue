<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>销售应付积分（按业务人）</el-breadcrumb-item>
		</el-breadcrumb>
    <div style="margin-top:10px;padding:10px;border:1px solid #ffffff;background-color:#ffffff;">
      <el-table v-for="d in listData" :data="d" style="width: 372px;display: inline-block;vertical-align: top;" size="mini" :stripe="true" :border="true">
    			<el-table-column prop="contacts_name" label="联系人" width="80" ></el-table-column>
  				<el-table-column prop="rsm" label="应付金额" width="100" :formatter="formatterMoney"></el-table-column>
...
<script>
  export default({
    ...
    activated(){
      this.listData = [];
      this.getSalesRefundMoneyPay();
    },
    methods:{
      viewReturnDetail(row){

        this.$router.push({path:`/main/reportSalesReturnMoneyPayDetail`,query:{contactId:row.contacts_id}});
      },
      formatterMoney(row, column, cellValue){
        return Math.round(cellValue*100)/100;
      },
      getSalesRefundMoneyPay(){
        var _self = this;
        _self.listData=[];
				this.jquery('/iae/report/getSalesReturnPayByContacts',null,function(res){
