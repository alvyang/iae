...
function HandleDB(options){
    this.databaseFile = options && options.databaseFile || `../data/iae.db`;    // 数据库文件(文件路径+文件名)
    this.tableName = options && options.tableName || `drugs`;   // 表名
    this.db = null;    // 打开的数据库对象
}
// 连接数据库(不存在就创建,存在则打开)
HandleDB.prototype.connectDataBase = function() {
    let _self = this;
 ...
 * @used
 */
HandleDB.prototype.createTable = function(sentence) {
    let _self = this;
    return new Promise((resolve, reject) => {
   ...
   /**
     ...
     */
HandleDB.prototype.sql = function(sql, param, mode) {
    let _self = this;
